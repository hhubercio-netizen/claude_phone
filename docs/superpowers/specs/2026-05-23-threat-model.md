...
| TM-AUTH.3  | Single-phone-per-session — second phone refused (`SessionTaken`) while first attached (D1 default; strictly safer than kick-previous which would enable attacker takeover of legit user's connection). Behaviour exists in `registry.rs:78-84`; sub-spec 4.2 adds forward-looking test + comment annotation. | PARTIAL |
...
| TM-CODE.4  | TOCTOU sweep in `session/registry.rs` (insert/check/use sequences)                          | TODO   |
...
| ID         | Mitigation                                                                                  | Status |
|------------|---------------------------------------------------------------------------------------------|--------|
| TM-TEST.1  | cargo-fuzz target on ControlMessage / SessionToken / ApiKey parsing                          | TODO   |
