...
| TM-AUTH.7  | Structured auth-failure log with correlation ID                                             | GREEN (`wrapper_ws.rs::handler` and `phone_ws.rs::handler` generate a 16-hex `conn_id` via `session::short_id()` BEFORE any auth check, then thread it through every auth-failure branch — rate-limited, forbidden-origin, invalid-api-key, bad-token-format, session-not-found, and the `&'static str` taxonomy returned by `recv_phone_hello`; every branch emits a canonical `tracing::warn!` with `event="auth_failure"`, `conn_id`, `peer_ip`, `reason`, `route` and never threads the rejected secret (api_key bytes / token bytes) into the macro call. `tests/auth_failure_log_test.rs` pins the canonical shape on both routes, the `reason` taxonomy tokens, the secret-absence invariant, and the 16-lowercase-hex `conn_id` format — pinned independently so a future shape drift fails the structured-fields test before the format test. `deploy/fail2ban/filter.d/claude-phone.conf` failregex updated to match the `peer_ip` field; `scripts/fail2ban_filter_test.sh` sample mirrors the new shape; `scripts/security_invariants.sh` greps the new failregex literal so a future filter drift fails CI before deploy) |
...
| TM-SECRET.11 | Opaque error chain in gateway responses (no internal detail in 4xx/5xx body)                | GREEN (`GatewayError::into_response` carries a `// TM-SECRET.11` anchor and maps `Internal`/`Io` to the fixed body literal `"internal error"`, while routing the real anyhow/io chain through `tracing::error!` for the server log only; the 4xx variants emit their static thiserror Display string and nothing more. `tests/opaque_error_test.rs` reads the body bytes for every variant and asserts: literal-equality on the 4xx and 5xx bodies, no secret-shaped 43-char substring, no anyhow context layers, size cap of 64 bytes, and that a 50KB injected payload still produces the fixed literal) |
